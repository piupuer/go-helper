...
func RowChange(ops Options, e *canal.RowsEvent) {
	...
	if err != nil {
		log.WithRequestId(ops.ctx).Error("[binlog row change]compress err: %+v, %v", err, e)
		return
	}
	err = ops.redis.Set(ops.ctx, cacheKey, compress, 0).Err()
	if err != nil {
		log.WithRequestId(ops.ctx).Error("[binlog row change]set to redis err: %+v, %v", err, e)
	}
}
...
func PosChange(ops Options, pos mysql.Position) {
	...
	if err != nil {
		log.WithRequestId(ops.ctx).Error("[binlog pos change]err: %+v, %v", err, pos)
	}
}
